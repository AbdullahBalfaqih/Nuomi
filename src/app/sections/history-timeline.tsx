...
const TimelineItem = ({ year, event, isLast }: { year: string, event: string, isLast: boolean }) => {
  return (
    <div className="flex items-start gap-6 md:gap-8">
      ...
      >
        <p className="text-lg text-foreground">{event}</p>
      </motion.div>
    </div>
  );
};
...
export default function HistoryTimeline() {
  ...
  return (
    <section ref={ref} className="py-24 sm:py-32 bg-secondary">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="relative max-w-2xl mx-auto">
            <div className="absolute left-[19px] top-0 h-full w-px bg-border -z-1" />
            <motion.div 
              className="absolute left-[19px] top-0 h-full w-px bg-primary origin-top" 
              style={{ scaleY }}
            />
